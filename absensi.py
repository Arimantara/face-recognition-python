...
class Detector:
	def __init__(self, vs, outputPath):
		...
		self.root.wm_title("Aplikasi Absensi Mahasiswa")
	...
	def videoLoop(self):
		...
		try:
			...
			while not self.stopEvent.is_set():
				...
				# memngambil data mahasiswa mahasiswa dari tabel mahasiswa
				...
				if (profile!=None):
					...
					if(conf<=50):
						counter += 1
						# menaruh text ID dan nama dari mahasiswa yang dikenali identitasnya
						...
						if(status == True):
							...
							if(counter >= 5):
								...
								if(fotoIn==True):
									...
									tkm.showinfo("Info Absensi", "Berhasil melakukan check-in")
									fotoIn = False
								else:
									# info message box bahwa check-in gagal dan harus check-out terlebih dahulu
									tkm.showinfo("Info Absensi", "Gagal check-in, harap check-out terlebih dahulu")
						...
						if(statusOut == True):
							if(counter >= 5):
								...
								if(fotoOut==True):
									...
									# menyimpan foto hasil pengenalan wajah untuk check-in ke folder foto
									tkm.showinfo("Info Absensi", "Berhasil melakukan check-out")
									fotoOut = False
								else:
									# info message box bahwa check-out gagal dan harus check-in terlebih dahulu
									tkm.showinfo("Info Absensi", "Gagal check-out, harap check-in terlebih dahulu")
								statusOut = False
							else:
								counter = 0
	...
	def checkIn(self):
		...
		if(conf<=50):			
			...
			tkm.showinfo("Info Absensi", "Gagal check-in, wajah tidak dikenali")
	...
	def checkOut(self):
		...
		if(conf<=50):
			...
			tkm.showinfo("Info Absensi", "Gagal check-out, wajah tidak dikenali")
